...
public class PersistenceWithNW extends PersistenceWithTemplate {
    ...
    protected void displayTable(HttpServletResponse response) throws SQLException, IOException {
        ...
        for (NW nw : resultList) {
        	double score = 0;
        	if (nw.getNw() != 0) {
        		score = nw.getNw() * 0.80 + (nw.getTotal()-nw.getNw())/nw.getNw() * 0.20 + 10;
        	}
        	
        	response.getWriter().println(
                    "<tr><td height=\"30\"><center>" + (index++) + "</center></td>"
                    + "<td height=\"30\"><center>" + xssEncoder.encodeHTML(nw.getName()) + "</center></td>"
					+ "<td><center><form action=\"" + LINKNAME + "?Id="+ nw.getId() + "&operation=add\" method=\"post\">" + "<input type=\"submit\" value=\"Add\" />" + "</form></center></td>" 
					+ "<td><center><form action=\"" + LINKNAME + "?Id="+ nw.getId() + "&operation=decrease\" method=\"post\">" + "<input type=\"submit\" value=\"Delete\" />" + "</form></center></td>" 
					+ "<td height=\"30\"><center>" + nw.getNw() + "</center></td>" // need to change to xssEncoder for getAmount()?
//					+ "<td height=\"30\"><center>" + nw.getSum() + "</center></td>" // need to change to xssEncoder for getSum()?
//					+ "<td height=\"30\"><center>" + score + "</center></td>"
						
					+ "<td><center><form action=\"" + LINKNAME + "?Id="+ nw.getId() + "&operation=ignore\" method=\"post\">" + "<input type=\"submit\" onclick=\"return window.confirm('This person will not be shown today!')\" value=\"ignore\" />" + "</form></center></td>" 
					);
